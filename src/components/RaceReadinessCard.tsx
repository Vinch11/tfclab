...
function computeReadiness(snapshot: DbSnapshot, objectif: string) {
  ...
  // Scores /25
  const vlamaxScore =
    vlamax == null ? 10 : vlamax >= 0.25 && vlamax <= (objectif === "IM" ? 0.45 : 0.55) ? 25 : vlamax < 0.25 ? 8 : 15;

  const puissanceScore =
    ftpKg == null ? 10 : ftpKg >= ftpTarget ? 25 : Math.round(clamp((ftpKg / ftpTarget) * 25, 5, 24));

  const enduranceScore =
    snapshot.metabolic_score != null
      ? Math.round(clamp(snapshot.metabolic_score / 4, 5, 25))
      : Math.round(clamp(15 + 10 * (conf - 0.5), 8, 22));
...
function texteExplicatif(snapshot: DbSnapshot, objectif: string) {
  ...
  lines.push("");
  lines.push(
    "Interprétation : ce score combine profil glycolytique (VLamax), puissance spécifique (FTP/kg), endurance (proxy) et qualité des données (confiance).",
  );
  return lines.join("\n");
}
