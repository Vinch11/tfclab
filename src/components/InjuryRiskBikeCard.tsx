/**
 * Carte Risque Blessure Cyclisme (Vélo)
...
import { AlertTriangle, Bike, ShieldAlert, Lightbulb, Info } from "lucide-react";
...
interface InjuryRiskBikeCardProps {
  riskEnvelope: InjuryRiskEnvelope;
  isStaffMode?: boolean;
  className?: string;
}

export function InjuryRiskBikeCard({ riskEnvelope, isStaffMode = false, className }: InjuryRiskBikeCardProps) {
  ...
  return (
    <Card className={cn("overflow-hidden", className)}>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base flex items-center gap-2">
            ...
            <span>Risque Blessure Vélo</span>
          </CardTitle>
          <V2ConfidenceBadge confidence={confidence} size="sm" />
        </div>
        <CardDescription className="text-xs">
          Indice composite • Cyclisme
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {/* Comparaison avec CAP si risque vélo faible */}
        {level === 'FAIBLE' && (
          <div className="p-2 rounded bg-green-500/10 border border-green-500/20 text-xs text-green-700 dark:text-green-300">
            💡 Risque vélo faible : possibilité de reporter une partie de la charge CAP sur vélo si besoin.
          </div>
        )}
