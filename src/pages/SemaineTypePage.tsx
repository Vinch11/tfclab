// =============================================
// ÉCRAN 5 - SEMAINE TYPE MULTI-SPORT
// =============================================
...
import { Copy, Calendar, AlertCircle, Dumbbell, Coffee, Bike, PersonStanding, Waves } from "lucide-react";
...
export default function SemaineTypePage() {
        ...
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground">
              Ajoutez des données pour générer la semaine
            </p>
  ...
  const getSportIcon = (sport: SportType) => {
    switch (sport) {
      case "vélo": return <Bike className="h-4 w-4 text-primary" />;
      case "course": return <PersonStanding className="h-4 w-4 text-accent" />;
      case "natation": return <Waves className="h-4 w-4 text-blue-400" />;
    }
  };

  const getSportColor = (sport: SportType) => {
    switch (sport) {
      case "vélo": return "bg-primary/10 border-primary/20";
      case "course": return "bg-accent/10 border-accent/20";
      case "natation": return "bg-blue-400/10 border-blue-400/20";
    }
  };

  const handleCopy = () => {
    const text = semaine.semaine
      .map((j) => `${j.jour} (${j.sport}): ${j.nom || j.type} - ${j.objectif}`)
      .join("\n");
